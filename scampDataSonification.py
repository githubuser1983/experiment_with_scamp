...
def constructData():
    ...
    return data,[mapping[t] for t in iris.target]

data,target = constructData()
index = 0

def scamp_loop():
   global countBass,tracks,oneOctave,SYMFUNC,NFUNC,BASEFUNC, counters, listFuncs,number_of_counters_x,s,started_transcribing, data, index
   ...
   while True:
      nTracks = len(tracks)
      barNumbers = data[index%len(data)] #[counters[k]  for k in range(nTracks)]
      ...
      if all([counters[3*number_of_counters_x+tt]>0 and counters[tt]>0 for tt in range(len(tracks))]): #all instruments played bar
          print("You were listening to ", target[index%len(data)], " represented by ", data[index%len(data)])
          if index == len(data)-1:
               return
